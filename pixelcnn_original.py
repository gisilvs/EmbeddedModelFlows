...
class PixelCNN(distribution.Distribution):
  ...
  def _make_mixture_dist(self, component_logits, locs, scales):
    ...
    normal_dist = quantized_distribution.QuantizedDistribution(
    ...
    dist = mixture_same_family.MixtureSameFamily(
        ...
        components_distribution=independent.Independent(
            normal_dist, reinterpreted_batch_ndims=1))
  ...
  def _sample_channels(
    ...
    for i in range(num_channels):
      ...
      normal_samp = normal.Normal(
          loc=loc, scale=scale_tensors[i]).sample(seed=seed)
      normal_samp = tf.clip_by_value(normal_samp, -1., 1.)
      channel_samples.append(normal_samp)
  ...
  def _default_event_space_bijector(self, *args, **kwargs):
    ...
    '''scale_bijector = scale.Scale(self._high - self._low)
    shift_bijector = shift.Shift(self._low)
